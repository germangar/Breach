...
{
	{ IDBSPHEADER, mod_IBSPQ2Versions, 0, 0, BSP_NOAREAS, Q2_LUMP_ENTITIES },
...
void Com_Printf( const char *format, ... )
{
	...
	// logfile
	if( logconsole && logconsole->string && strlen( logconsole->string ) )
	{
		if( !logfile )
		{
			int length;
			...
			Q_strncpyz( name, logconsole->string, sizeof( name ) );
			COM_DefaultExtension( name, ".log", sizeof( name ) );

			if( logconsole_append->integer )
				length = FS_FOpenFile( name, &logfile, FS_APPEND );
			else
				length = FS_FOpenFile( name, &logfile, FS_WRITE );

			if( length == -1 )
			{
				logfile = -1; // only try to open it once
...
void Com_Error( int code, const char *format, ... )
{
	...
	if( recursive )
	{
		Com_Printf( "recursive error after: %s", msg ); // log it
...
void Qcommon_Init( int argc, char **argv )
{
	...
	// Kurim : default file won't exist anymore
