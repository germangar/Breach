...
qboolean SNDDMA_Init( void *hwnd, qboolean verbose )
{
	...
	// jal : dirty trick to let the sound module know about hWnd without having to guess it
	//cvar_t *win_hWnd = trap_Cvar_Get( "win_hWnd", "", 0 );
	//sscanf( win_hWnd->string, "%i", (int *)&cl_hwnd );
	cl_hwnd = ( HWND )hwnd;
