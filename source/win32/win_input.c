...
#ifdef __GNUC__
#define	DIRECTINPUT_VERSION 0x0300 // Could use dx9, but older is more frequently used
#else
...
void IN_RawInput_MouseRead( HANDLE in_device_handle )
{
	...
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_1_DOWN )
		Key_Event( K_MOUSE1, qtrue, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_1_UP )
		Key_Event( K_MOUSE1, qfalse, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_2_DOWN )
		Key_Event( K_MOUSE2, qtrue, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_2_UP )
		Key_Event( K_MOUSE2, qfalse, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_3_DOWN )
		Key_Event( K_MOUSE3, qtrue, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_3_UP )
		Key_Event( K_MOUSE3, qfalse, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_4_DOWN )
		Key_Event( K_MOUSE4, qtrue, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_4_UP )
		Key_Event( K_MOUSE4, qfalse, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_5_DOWN )
		Key_Event( K_MOUSE5, qtrue, sys_msg_time );
	if( raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_5_UP )
		Key_Event( K_MOUSE5, qfalse, sys_msg_time );
	...
	for( j = 6; j < rawmice[i].numbuttons; j++ )
	{
		if ( (tbuttons & (1<<j)) && !(rawmice[i].buttons & (1<<j)) )
			Key_Event (K_MOUSE1 + j, qtrue, sys_msg_time);

		if ( !(tbuttons & (1<<j)) && (rawmice[i].buttons & (1<<j)) )
			Key_Event (K_MOUSE1 + j, qfalse, sys_msg_time);
	}
...
void IN_Frame( void )
{
	...
	if( vid_fullscreen && !vid_fullscreen->integer )
	{
		if( in_grabinconsole->integer || cls.key_dest != key_console )
		{
		...
		case AxisForward:
			...
			else
			{
				...
				if( fabs( fAxisValue ) > joy_forwardthreshold->value )
					cmd->forwardmove += ( fAxisValue * joy_forwardsensitivity->value ) * JOY_SPEED_KEY;
			}
			break;

		case AxisSide:
			if( fabs( fAxisValue ) > joy_sidethreshold->value )
				cmd->sidemove += ( fAxisValue * joy_sidesensitivity->value ) * JOY_SPEED_KEY;
			break;

		case AxisUp:
			if( fabs( fAxisValue ) > joy_upthreshold->value )
				cmd->upmove += ( fAxisValue * joy_upsensitivity->value ) * JOY_SPEED_KEY;
			break;
			...
			else
			{
				...
				if( fabs( fAxisValue ) > joy_yawthreshold->value )
				{
					if( dwControlMap[i] == JOY_ABSOLUTE_AXIS )
						...
						cl.viewangles[YAW] += ( fAxisValue * joy_yawsensitivity->value ) * 180.0;
				}
			}
			break;
			...
			if( jlooking )
			{
				if( fabs( fAxisValue ) > joy_pitchthreshold->value )
				{
					...
					if( dwControlMap[i] == JOY_ABSOLUTE_AXIS )
						...
						cl.viewangles[PITCH] += ( fAxisValue * joy_pitchsensitivity->value ) * 180.0;
				}
			}
			break;
