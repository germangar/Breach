...
static void install_grabs( void )
{
	...
	{
		Com_Error( ERR_FATAL, "XGrabPointer failed" );
	}
...
static char *KeysymToKey( KeySym keysym )
{
	...
	case XK_Scroll_Lock: return "K_SCROLLOCK";
	}
...
static void HandleEvents( void )
{
	...
	while( XPending( x11display.dpy ) )
	{
		...
		case ButtonPress:
			time = Sys_XTimeToSysTime(event.xkey.time);
			if( event.xbutton.button == 1 ) Key_Event( K_MOUSE1, 1, time );
			else if( event.xbutton.button == 2 ) Key_Event( K_MOUSE3, 1, time );
			else if( event.xbutton.button == 3 ) Key_Event( K_MOUSE2, 1, time );
			else if( event.xbutton.button == 4 ) Key_Event( K_MWHEELUP, 1, time );
			else if( event.xbutton.button == 5 ) Key_Event( K_MWHEELDOWN, 1, time );
			else if( event.xbutton.button >= 6 && event.xbutton.button <= 10 ) Key_Event( K_MOUSE4+event.xbutton.button-6, 1, time );
			break;

		case ButtonRelease:
			time = Sys_XTimeToSysTime(event.xkey.time);
			if( event.xbutton.button == 1 ) Key_Event( K_MOUSE1, 0, time );
			else if( event.xbutton.button == 2 ) Key_Event( K_MOUSE3, 0, time );
			else if( event.xbutton.button == 3 ) Key_Event( K_MOUSE2, 0, time );
			else if( event.xbutton.button == 4 ) Key_Event( K_MWHEELUP, 0, time );
			else if( event.xbutton.button == 5 ) Key_Event( K_MWHEELDOWN, 0, time );
			else if( event.xbutton.button >= 6 && event.xbutton.button <= 10 ) Key_Event( K_MOUSE4+event.xbutton.button-6, 0, time );
			break;
