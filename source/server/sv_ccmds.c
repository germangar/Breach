...
static void SV_Kick_f( void )
{
	...
	// from r1q2: ignore kick message on connecting players (and those with no name)
	if( client->state < CS_CONNECTED || !client->name[0] )
		return;

	SV_ClientPrintf( NULL, "%s%s was kicked\n", client->name, S_COLOR_WHITE );
	SV_DropClient( client, DROP_TYPE_NORECONNECT, "You were kicked" );
