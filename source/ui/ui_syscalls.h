...
static inline void trap_Error( const char *str )
{
	UI_IMPORT.Error( str );
}

static inline void trap_Print( const char *str )
{
	UI_IMPORT.Print( str );
}
...
static inline int trap_R_SkeletalGetBoneInfo( struct model_s *mod, int bone, char *name, size_t name_size, int *flags )
{
	return UI_IMPORT.R_SkeletalGetBoneInfo( mod, bone, name, name_size, flags );
}
...
{
	return UI_IMPORT.GetConfigString( i );
}
...
{
	return UI_IMPORT.FS_Print( file, msg );
}
