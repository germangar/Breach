...
void UI_ColorRedCallback( struct menuitem_public_s *menuitem )
{
	playerColor[0] = menuitem->value;
}
void UI_ColorGreenCallback( struct menuitem_public_s *menuitem )
{
	playerColor[1] = menuitem->value;
}
void UI_ColorBlueCallback( struct menuitem_public_s *menuitem )
{
	playerColor[2] = menuitem->value;
}
...
static char **UI_Playermodels_ScanDirectory( char *model, int *numSkins )
{
	...
	*numSkins = 0;
	...
	skinnames = UI_Malloc( sizeof( char * ) * ( skincount + 1 ) );
	nskins = 0;
...
static void UI_Playermodels_CreateItemList( m_itemslisthead_t *itemlist )
{
	...
	do
	{
		...
		for( dirptr = dirnames; k > 0; k--, dirptr += dirlen+1 )
		{
			...
			{                                                                   // put it in the list
				playermodelinfo_s *playermodel = UI_Malloc( sizeof( playermodelinfo_s ) );
