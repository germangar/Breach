...
BEGIN_MESSAGE_MAP(CMudFontDlg, CDialog)
	...
	ON_COMMAND(ID_TOOLS_ADDWSW, OnToolsAddWarsowCharset)
...
void CMudFontDlg::OnToolsAddWarsowCharset() 
{
...
*/
 ...
 // wsw : jal : warsow font
void CMudFontDlg::ExportWSW(const char* a_Filename)
{
	char path[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
	char fname[_MAX_FNAME];
	_splitpath(a_Filename, drive, dir, fname, NULL);
	_makepath(path, drive, dir, fname, ".wfd");
